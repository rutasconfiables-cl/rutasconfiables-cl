...
export default async function DestinationPageEn({ params }: Props) {
  ...
  return <main className="destination-page">
    ...
    <section className="destination-hero" style={{backgroundImage:`linear-gradient(90deg,rgba(0,20,58,.96),rgba(0,25,65,.54),rgba(0,20,58,.1)),url(${d.image})`}}><div><p>Private experience in Chile</p><h1>{d.title}</h1><span>{detail.description}</span><a href={quote} className="destination-quote"><MessageCircle size={21}/> Request a quote</a></div></section>
    ...
    <section className="destination-bottom"><h2>Ready to discover {d.title}?</h2><p>Tell us your group size and preferred date.</p><a href={quote}><MessageCircle size={22}/> Chat on WhatsApp</a></section>
