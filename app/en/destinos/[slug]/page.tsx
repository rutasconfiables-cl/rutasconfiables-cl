...
export default async function DestinationPageEn({ params }: Props) {
  ...
  return <main className="destination-page">
    ...
    <section className="destination-hero" style={{backgroundImage:`linear-gradient(90deg,rgba(0,20,58,.96),rgba(0,25,65,.54),rgba(0,20,58,.1)),url(${d.image})`,backgroundPosition:d.slug === "termas-de-colina" ? "center bottom" : undefined}}><div><p>Private experience in Chile</p><h1>{d.title}</h1><span>{detail.description}</span><SafetyBadge locale="en"/><a href={quote} className="destination-quote"><WhatsAppIcon/> Request a quote</a></div></section>
