...
export default async function DestinationPage({ params }: Props) {
  ...
  return <main className="destination-page">
    <DestinationHeader/>
    <section className="destination-hero" style={{backgroundImage:`linear-gradient(90deg,rgba(0,20,58,.96),rgba(0,25,65,.54),rgba(0,20,58,.1)),url(${destination.image})`,backgroundPosition:destination.slug === "termas-de-colina" ? "center bottom" : undefined}}><div><p>{destination.eyebrow}</p><h1>{destination.title}</h1><span>{detail.description}</span><SafetyBadge/><a href={quote} className="destination-quote" target="_blank" rel="noreferrer"><WhatsAppIcon/> Cotizar este viaje</a></div></section>
